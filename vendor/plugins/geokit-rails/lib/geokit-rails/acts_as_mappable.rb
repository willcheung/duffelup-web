module Geokit
  ...
  module ActsAsMappable 
    ...
    module ClassMethods # :nodoc:
      ...
      def acts_as_mappable(options = {})
        ...
        if options.include?(:auto_geocode) && options[:auto_geocode]
          ...
          # set the actual callback here (customized by Will C.)
          before_save :auto_geocode_address    
        end
      end
    end
    
    # this is the callback for auto_geocoding - Hacked by Will C.
    def auto_geocode_address
      address=self.send(auto_geocode_field).to_s
      
      return true unless address_changed? # return normally if address is not changed
      
      geo=Geokit::Geocoders::MultiGeocoder.geocode(address) unless address.empty?
  
      # if address is empty, then don't geocode
      if geo.nil?
        self.send("city_id=", "0")
        return true
      end
      ...
      if geo.success
        ...
        # Customized by Will C.
        self.send("city_id=", geocode_to_city_id(geo))
      else
        # Customized by Will C.
        #errors.add(auto_geocode_field, auto_geocode_error_message) 
        # Failed geocode will still be saved.
        self.send("city_id=", "0")
        geo.success = true
      end
    ...
    # Custom extension by Will C. - gets city_id from existing 'cities' table
    def geocode_to_city_id(geo)
      if geo.city.nil? or geo.city.empty? or geo.country_code.nil? or geo.country_code.empty?
        return "0"
      else
        city = geo.city.downcase
        country_code = geo.country_code
      end
    
      cities = City.find_by_sql("SELECT cities.id FROM cities INNER JOIN countries on cities.country_id = countries.id WHERE (LOWER(cities.city) = \"" + city + "\") and countries.country_code = \"" +  country_code + "\"")
      if cities.empty?
        return "0"
      else
        return cities[0].id.to_s
      end
    end
