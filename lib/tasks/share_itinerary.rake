namespace :share_itinerary do
  ...
  task :send_emails => :environment do
    ...
    users_trips = User.find_by_sql(["SELECT users.id, users.username, users.email, users.full_name, users.email_updates, trips.id as trip_id, trips.title as trip_title, trips.permalink as trip_perma, trips.is_public as trip_is_public, trips.destination as trip_dest
                                      ...
                                      WHERE trips.end_date = FROM_DAYS(TO_DAYS(CURRENT_DATE)-?) order by permalink", num_of_days])
                                      
    puts "Time now is " + Time.now.to_s + " and we're sending " + users_trips.size.to_s + " emails to share itinerary."
    puts "--------------- end ------------------"
    
    for u in users_trips
      if user_is_subscribed(u.email_updates, User::EMAIL_TRIP_REMINDER)
        Postoffice.deliver_share_itinerary_after_trip(u)
      end
      ...
      ActivitiesFeed.insert_activity(user, ActivitiesFeed::RETURN_FROM_TRIP, trip)
    end
